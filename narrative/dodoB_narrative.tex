...
data management using principles from data science. This last barrier
we address with substantial curriculum development, detailed in
section \ref{training-and-mentoring} on training and Broader Impacts.
...
Because we selected five mature DoB projects to populate our
bioinformatics pipeline and test our model, many data resources are
already available; our new data collection efforts will focus on
filling specific gaps to ensure continuity of data types across
projects. Table \ref{tab:data} summarizes available data and gaps to
be filled.
...
The primary information gap in the Atlantic
Forest are curated data quantifying the abundance of amphibians
(within a group of $\approx 100$ Hylidae tree frogs within Boana,
and the Phyllomedusinae), lizards (within the Ecpleopodinae), butterflies
...
well studied and we will use these results to implement efficient
forward time simulations \cite{Haegeman2017-kf,Kendall1948-ri}.  These
forward-time assemblage histories will feed into aggregate population
genetic histories that can be efficiently simulated via extensions of
the Kingman coalescent \cite{Rosenberg2002-vb}. We will use msprime
\cite{Kelleher2016-an} to implement this coalescent approach to
simulating population genetic data. These simulations depend on the
time trajectory of effective population size ($N_e$) for each species
and the mutation rate $\mu$. The time trajectory of $N_e$ for a
species is determined by that species' simulated population
fluctuations \cite{waples2007}, which in turn derive from the
parameters of birth, death, and immigration, which are fit to the
data.  Mutation rate $\mu$ is a free parameter that will also be fit
to the data.
